...
func main() {
    ...
    // utxos = methods.LoadUtxos(mmap["utxos"], utxos)
    ...
    for {
        ...
        go Handle(mmap, conn, bufmax)// why give the cinfig file and the bufmax knowing that bufmax is included in the conf file
    }
}
...
func Handle(config map[string]string, conn net.Conn, maxbuf int) {
    ...
    for {
          ...
          structures.PrintTransaction(tx, "master Pay In")
          if n == maxbuf {
    ...
    case "PayCoins":
      if rerr == nil {
        if msg, ok := methods.LegitimateTransaction(utxos, px); ok {
          ...
          fmt.Printf("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
          structures.PrintCoins(utxos, "Utxos before failure")
          parsee = Confirm("PayeeReceipt", conf, px, true)
