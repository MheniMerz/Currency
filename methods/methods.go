...
//check id a gtring is in a list of strings
...
func GetDir(dirname string, suffix string) (fileslice []string) {
  ...
  if err != nil {
    log.Fatal(fmt.Sprintf("'%s': %s", dirname, err))
  } // endif.
...
func ProcessNewTransactions(config map[string]string, lodger []structures.Transaction) ([]structures.Transaction, []structures.Transaction, []structures.Coin) {

  newin := []structures.Transaction{}
  newout := []structures.Transaction{}
  rejects := []structures.Transaction{}
  emmy := []structures.Coin{}
